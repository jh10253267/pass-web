...
import com.example.passweb.service.statistics.AggregatedStatistics;
...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity, Integer> {
    ...
    @Query(value = "SELECT new com.example.passweb.service.statistics.AggregatedStatistics(s.statisticsAt, SUM(s.allCount), SUM( s.attendedCount), SUM(s.cancelledCount))" +
